      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        <div ref={ref} className="text-center mb-14">
          <motion.p className={`text-xs tracking-[0.3em] uppercase mb-3 ${ml ? 'font-malayalam tracking-normal' : 'font-poppins'}`}
            style={{ color: 'rgba(139,74,42,0.7)' }} initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}>
            {t.gallery.label}
          </motion.p>
            ...
            return (
              <motion.div key={i} ref={imgRef}
                ...
                <div className="absolute inset-0 flex items-center justify-center"
                  style={{ background: `linear-gradient(${135 + i * 30}deg, rgba(139,74,42,0.12), rgba(122,48,64,0.08))` }}>
                  <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#8B4A2A" strokeWidth="1.2" opacity="0.45">
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center"
                  style={{ background: 'rgba(139,74,42,0.25)' }}>
          ...
          <motion.div className="fixed inset-0 z-150 flex items-center justify-center p-4"
            ...
            <motion.div key={selected} className="relative max-w-3xl max-h-[80vh] w-full"
              ...
              <div className="relative rounded-2xl overflow-hidden"
                style={{ aspectRatio: '4/3', border: '1px solid rgba(139,74,42,0.25)', background: '#F2E8E3' }}>
                <Image src={images[selected]} alt={`Gallery ${selected + 1}`} fill className="object-contain" />
              </div>
              <p className={`text-center mt-3 text-sm ${ml ? 'font-malayalam' : 'font-poppins'}`} style={{ color: 'rgba(139,74,42,0.7)' }}>
