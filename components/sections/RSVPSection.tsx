      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6">
        <div ref={ref} className="text-center mb-12">
          <motion.p className={`text-xs tracking-[0.3em] uppercase mb-3 ${ml ? 'font-malayalam tracking-normal' : 'font-poppins'}`}
            style={{ color: 'rgba(212,175,55,0.75)' }} initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}>
            {t.rsvp.label}
          </motion.p>
          ...
          <motion.p className={`text-sm ${fontClass}`} style={{ color: 'rgba(255,253,247,0.55)' }}
        ...
        <AnimatePresence mode="wait">
          {submitted ? (
            <motion.div key="success" className="text-center py-16"
              ...
              <p className={`text-sm ${fontClass}`} style={{ color: 'rgba(255,253,247,0.6)' }}>{t.rsvp.successMsg}</p>
            </motion.div>
          ) : (
            <motion.form key="form" onSubmit={handleSubmit}
              className="rounded-3xl p-8 space-y-5"
              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(212,175,55,0.18)', backdropFilter: 'blur(20px)' }}
              initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.5 }}>
              {/* Name */}
              <div>
                <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.75)' }}>{t.rsvp.nameLabel}</label>
              ...
              {/* Phone */}
              <div>
                <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.75)' }}>{t.rsvp.phoneLabel}</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.75)' }}>{t.rsvp.guestsLabel}</label>
                ...
                <div>
                  <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.75)' }}>{t.rsvp.attendingLabel}</label>
                      ...
                      <button key={val} type="button" onClick={() => update('attending', val)}
                        ...
                        style={{
                          background: form.attending === val
                            ? (val === 'yes' ? 'linear-gradient(135deg, #D4AF37, #C5A028)' : 'rgba(239,68,68,0.15)')
                            : 'rgba(255,255,255,0.05)',
                          border: form.attending === val
                            ? (val === 'yes' ? '1px solid #D4AF37' : '1px solid rgba(239,68,68,0.4)')
                            : '1px solid rgba(212,175,55,0.2)',
                          color: form.attending === val
                            ? (val === 'yes' ? '#0F5132' : '#dc2626')
              ...
              {/* Message */}
              <div>
                <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.75)' }}>{t.rsvp.messageLabel}</label>
              ...
              <motion.button type="submit"
                ...
                style={{ background: 'linear-gradient(135deg, #D4AF37, #C5A028)', color: '#0F5132', boxShadow: '0 8px 32px rgba(212,175,55,0.25)' }}
