...
export default function RSVPSection({ config }: Props) {
  ...
  return (
    <section id="rsvp" className="relative py-24 md:py-32 overflow-hidden"
      style={{ background: 'linear-gradient(160deg, #0F5132, #0a3d24 50%, #133d26)' }}>
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6">
        <div ref={ref} className="text-center mb-12">
          ...
          <motion.p className={`text-sm ${fontClass}`} style={{ color: 'rgba(255,253,247,0.6)' }}
          ...
          {submitted ? (
            <motion.div key="success" className="text-center py-16"
              ...
              <motion.div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6"
                ...
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#0F5132" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
              </motion.div>
              <h3 className={`text-2xl font-bold mb-3 ${fontClass}`} style={{ color: '#D4AF37' }}>{t.rsvp.successTitle}</h3>
              <p className={`text-sm ${fontClass}`} style={{ color: 'rgba(255,253,247,0.7)' }}>{t.rsvp.successMsg}</p>
            </motion.div>
          ) : (
            <motion.form key="form" onSubmit={handleSubmit}
              className="rounded-3xl p-8 space-y-5"
              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(212,175,55,0.2)', backdropFilter: 'blur(20px)' }}
              initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.5 }}>
              {/* Name */}
              <div>
                <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.7)' }}>{t.rsvp.nameLabel}</label>
              ...
              {/* Phone */}
              <div>
                <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.7)' }}>{t.rsvp.phoneLabel}</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.7)' }}>{t.rsvp.guestsLabel}</label>
                ...
                <div>
                  <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.7)' }}>{t.rsvp.attendingLabel}</label>
                  <div className="flex gap-2 h-[46px]">
                    {(['yes', 'no'] as const).map((val) => (
                      <button key={val} type="button" onClick={() => update('attending', val)}
                        ...
                        style={{
                          background: form.attending === val ? (val === 'yes' ? 'linear-gradient(135deg, #D4AF37, #C5A028)' : 'rgba(239,68,68,0.3)') : 'rgba(255,255,255,0.07)',
                          border: form.attending === val ? (val === 'yes' ? '1px solid #D4AF37' : '1px solid rgba(239,68,68,0.5)') : '1px solid rgba(212,175,55,0.2)',
                          color: form.attending === val ? (val === 'yes' ? '#0F5132' : '#fca5a5') : 'rgba(255,253,247,0.6)',
                        }}>
              ...
              {/* Message */}
              <div>
                <label className={`block text-xs tracking-wider uppercase mb-2 ${fontClass}`} style={{ color: 'rgba(212,175,55,0.7)' }}>{t.rsvp.messageLabel}</label>
              ...
              <motion.button type="submit"
                ...
                style={{ background: 'linear-gradient(135deg, #D4AF37, #C5A028)', color: '#0F5132', boxShadow: '0 8px 32px rgba(212,175,55,0.3)' }}
